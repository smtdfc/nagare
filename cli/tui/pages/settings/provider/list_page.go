...
func (m *ListPage) Refresh() {
	...
	for _, name := range names {
		...
		status := ""
		if m.config.CurrentProvider == name {
			status = "• Active"
		}
...
func (m *ListPage) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter":
			...
			if len(selected) > 1 {
				providerName := selected[1]
				m.config.CurrentProvider = providerName
...
func (m *ListPage) View() tea.View {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("205")).Render("Active Provider Selection")
	helpMsg := lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Render("Press [Enter] to select active provider • Press [Esc/q] to go back")
