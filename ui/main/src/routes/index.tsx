...
import { AgentResponseStatus } from "@nagare-agent/dto";
import {
  isAgentResponseMessage,
  isResponseFailedMessage,
  isTextMessage,
  isToolCallMessage,
...
function Home() {
  ...
  const addMessage = useCallback(
    (message: MessageItem, forceNew: boolean = false) => {
  ...
  useEffect(() => {
    ...
    const setupChat = async () => {
      try {
        ...
        unsubscribe = ChatService.listenMessage(
          (_isSuccess: boolean, messageChunk?: Message, err?: string) => {
            if (!messageChunk) {
              addMessage(
                {
                  id: `system_${Date.now().toString()}`,
                  role: "agent",
                  content: "Failed to sent message",
                  isErrMessage: true,
                },
                mustNewMessage,
              );
              ...
              toast.add({
                title: "Error",
                description: err ?? "Failed to sent message",
                type: "error",
              });
              setStatus(Status.IDLE);
              return;
            ...
            if (
              isAgentResponseMessage(messageChunk) &&
              messageChunk.status ===
                AgentResponseStatus.AGENT_RESPONSE_COMPLETED
            ) {
              mustNewMessage = true;
              setTimeout(() => {
                messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
            ...
            if (isResponseFailedMessage(messageChunk)) {
              addMessage(
                {
                  id: `system_${Date.now().toString()}`,
                  role: "agent",
              ...
              toast.add({
                title: "Error",
                description: messageChunk.cause ?? "Failed to sent message",
                type: "error",
              });
              setStatus(Status.IDLE);
              return;
